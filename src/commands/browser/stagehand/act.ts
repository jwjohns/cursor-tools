...
import { BrowserResult, ConstructorParams, LogLine, Stagehand } from '@browserbasehq/stagehand';
...
const dumbMessages = new Set(["clicked element", "running / continuing action", "looking at chunk", "creating chat completion", "processing DOM", "transformed response"])
const boringMessages = new Set(["launching local browser", "local browser started successfully.", "action completed successfully", "finished waiting for (possible) page navigation"])
...
export class ActCommand implements Command {
    ...
    try {
      ...
      await using _stagehand = {
          [Symbol.asyncDispose]: async () => {
              console.error("closing stagehand");
              await Promise.race([
                  stagehand?.close(),
                  new Promise((_, reject) =>
                    setTimeout(() => reject(new Error('Page close timeout')), 5000)
      ...
      // Initialize with timeout
      const initPromise = stagehand.init({ // this method is overriden in our Stagehand class patch hack
        ...options,
        //@ts-ignore
      ...
      const result = await this.performAction(stagehand, { instruction: query, evaluate: options?.evaluate }, options?.timeout ?? stagehandConfig.timeout );
      actionSuccessful = true;
  ...
  private async performAction(
    ...
    try {
      ...
      if (evaluate) {
        await Promise.race([
          stagehand.page.evaluate(evaluate),
          totalTimeoutPromise
        ]);
      }
      ...
      if (error instanceof Error) {
        throw new ActionError(
          `${error.message} Failed to perform action: ${instruction}`,
          {
            instruction,
            error,
...
//@ts-ignore
Stagehand.prototype.init = async function myinit(initOptions: undefined | { recordVideo: RecordVideoOptions, viewport?: SharedBrowserCommandOptions['viewport'], connectTo?: SharedBrowserCommandOptions['connectTo'] }) : Promise<InitResult> {
...
function stagehandLogger(verbose: boolean): ((message: LogLine) => void | Promise<void>) | undefined {
  return (message) => {
...
async function getBrowser(
  ...
  logger: (message: LogLine) => void,
    ...
    if(options.connectTo) {
     ...
     else { context = await chromium.launchPersistentContext(
